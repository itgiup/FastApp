...
export abstract class GasFree {
  ...
  public addFunctionSelectorToAddress(address: string): string {
    ...
    const funcSelector = keccak256(toUtf8Bytes(functionSignature)).slice(0, 10)
    // const funcSelector = hexlify(selector).slice(0, 10);
  ...
  public calculateBytecodeHash(address: string, beacon: string, creationCode: string): string {
    ...
    const encodedData = concat([
      toBeArray(creationCode),
      toBeArray(coder.encode(
        ['address', 'bytes'],
        [beaconAddress, toBeArray(initializeData)]
      )),
    ]);

    return hexlify(keccak256(encodedData));
  }
  ...
  public calculateCreate2Address(
    ...
    const create2Input = concat([
      toBeArray(this.getCreate2PrefixByte()),
      toBeArray(gasFreeControllerHex),
      toBeArray(salt),
      toBeArray(bytecodeHash),
    ]);
    const ethAddress: EvmAddress = `0x${hexlify(keccak256(create2Input)).slice(26)}`;
    return ethAddress;
  }
  ...
  public getGasFreeLedgerRawHash({
    ...
    const hashStructMessageHex =
      '0x' +
      TypedDataEncoder.hashStruct('PermitTransfer', {
        PermitTransfer,
      }, message as unknown as Record<string, unknown>);

    const permitTransferMessageHash = hexlify(
      keccak256(
        concat([
          toBeArray('0x1901'),
          toBeArray(domainSeparatorHex),
          toBeArray(hashStructMessageHex),
        ]),
      ),
    );
