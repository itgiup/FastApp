...
import { type AssembleGasFreeTransactionParams, GasFree, type GasFreeParameter } from './GasFree';
...
export class TronGasFree extends GasFree {
  ...
  protected checkIsValidAddress(address: string | undefined): boolean {
    ...
    return TronWeb.utils.crypto.isAddressValid(address) || isAddress(address);
  }

  public generateGasFreeAddress(userAddress: string): TronAddress {
    if (!TronWeb.utils.crypto.isAddressValid(userAddress)) {
