...
if TYPE_CHECKING:
    from fastapp.app.schemas import PaginationParams, SortingParams
...
class ShortUrl(Document):
    ...
    async def shorten(
        ...
        *,
        ...
        return await cls(
            ...
            slug=slug,
            expires_at=(
                datetime.now(datetime.timezone.utc) + timedelta(days=expiration_days)
                if expiration_days
                else None
            ),
            user_id=user_id,
    ...
    async def visit(cls, *, instance: "ShortUrl") -> None:
        instance.views += 1
        instance.last_visit_at = datetime.now(datetime.timezone.utc)
